...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=20):
	...
	g_model.save(os.path.join('pretrained','fmri3dcgan_embeds_1k.h5'))

# generate images
def generate_pred_pairs(model, labels=None):
    latent_points, _ = generate_latent_points(1000, 2)
    if not labels.any(): labels = _
...
# load model
model = load_model(os.path.join('pretrained','fmri3dcgan_embeds_1k.h5'))
...
for i in range(5):
    custom_y = lencoder.transform(custom_labels[i])
    ...
    all_predictions += [[predictions, trainX[predy,:,:,:,0], custom_labels[i]]]
...
#%% ---------------------------------------------------------------------
for i in range(5):
    p1_gen = all_predictions[i][0][0]
    p1_real = all_predictions[i][1][0]
    lbl = all_predictions[i][2][0]

    sample_vec = postprocess.img2vector(p1_real, voxel_map)
    vmin = np.min(p1_real)
    vmax = np.max(p1_real)

    sample_image = fmriviz.prepare_image(sample_vec, voxel_map, fill=vmin)
    fmriviz.plot_slices(sample_image,vmin,vmax, filename="GAN_" + lbl + "_real")
    # pickle.dump(p1_real, open("GAN_"+lbl+"_real", "wb"))
    ...
    # pickle.dump(p1_gen, open("GAN_"+lbl+"_gen", "wb"))
